	...
	$.fn.treeview = function(settings) {
		...
		function buildnode(nd, ht, deep, path, isend, source, pid) {
			...
			if (deep > 0) {
				...
				getParents(nd,pids,function(item){return item.id})
			}
			...
			ht.push("<div class='bg-node' style='margin-left:", itemMargin,
					"px;width:");
			if (deep < 1) {
				//ht.push(me.width() - itemMargin);
				ht.push("100%");
			} else if(deep <2){
				//ht.push(me.width() - itemMargin - 20);
				ht.push("97%");
			}else{
				ht.push("94%");
			}
			...
			//span indent
			ht.push('<ul class="node-row" >');
			ht.push('<li style="text-align:left;padding-left:', itemMargin,
					'px;">');
		...
		function nodeclick(e) {
			...
			if (et.tagName == "IMG") {
				...
				//if expended, collapse it
						...
						|| $(et).hasClass("bbit-tree-elbow-end-minus")) {
					$(this).next().hide();
...
//					}
		...
		function getSearchData(items, key, c, parent, pid) {
			...
			for ( var i = 0, l = items.length; i < l; i++) {
				...
				if (pid) {
					...
					if (items[i].pinyin == key || items[i].text == key
							|| items[i].jianpin == key) {
						c.push(parent);
					...
					} else if (items[i].ChildNodes != null
							&& items[i].ChildNodes.length > 0) {
						getSearchData(items[i].ChildNodes, key, c, items[i]);
					}
					var flags = [], flag = false;
					isContain(items[i].id, c, flags);
					if (flags.length > 0) {
						flag = true;
					}
					if (items[i].pinyin != key && items[i].text != key
							&& items[i].jianpin != key && !flag) {
						//  	if(!items[i].ChildNodes || items[i].ChildNodes.length == 0){
		...
		function clickSearch(searchData,c) {
			for ( var i = 0, l = searchData.length; i < l; i++) {
				...
				if(flag){
					if (searchData[i].ChildNodes != null
							&& searchData[i].ChildNodes.length > 0) {
